# Examining Dynamic Structure of a Program

The execution `make dev` of the `find-and-replace` program dumps the dynamic structure of the program in "Lisp format". The dump can be pretty-printed by a Lisp pretty printer (in my case Emacs lisp-mode "indent-region"). This formatted version shows the live relationships between components. We use `◦1` syntax to uniquely identify component instances. Components can be used multiple times in a Design, then the components are instantiated at runtime.
